...
const SimpleImageUpload = () => {
  ...
  const handleImageUpload = (e: ChangeEvent<HTMLInputElement>) => {
    ...
    // Reset image loaded state when a new image is uploaded
  ...
  // New function for iOS compatibility
  const buildPng = async () => {
    ...
    while (dataUrl.length < minDataLength && i < maxAttempts) {
      dataUrl = await toPng(inviteRef.current, { quality: 0.95 });
      i += 1;
    }
  ...
  const downloadInvite = async () => {
    ...
    try {
      ...
      if (dataUrl) {
        saveAs(dataUrl, "e-invite.png"); // Changed extension to .png
      } else {
      ...
      <div className="flex w-full flex-col md:flex-row gap-8">
        <div className="w-full md:w-1/3 bg-gray-50 p-4 rounded-lg shadow">
          ...
          <button
            ...
            disabled={!userImage || !isImageLoaded || isDownloading}
